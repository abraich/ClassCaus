...
class ClassifCaus(nn.Module):
    ...
    def eval_all_test(self, N):
        ...
        pehe = metrics.mean_squared_error(cate_true, cate_pred)
...
class BenchmarkClassif():
    ...
    def fit_model(self, model_name):
        ...
        model_base_0.fit(self.data.x_train_0, self.data.y_train_0.view(-1))
        model_base_1.fit(self.data.x_train_1, self.data.y_train_1.view(-1))
...
d_0, d_1 = classifcaus.eval_all_test()

d = {'d_0': d_0, 'd_1': d_1}
# to dataframe
df = pd.DataFrame(d)
df.to_csv('classif_caus.csv')
print(df)
"""
"""print(classifcaus.data.x_train_0.shape)
print(classifcaus.data.x_train_1.shape)
print(classifcaus.data.y_train_0)
print(classifcaus.data.counter_train_0.shape)

clf = lgbm.LGBMClassifier()

clf.fit(classifcaus.data.x_train_0, classifcaus.data.y_train_0.view(-1))


print(classification_report(classifcaus.data.y_train_0, clf.predict(classifcaus.data.x_train_0)))
print(classification_report(classifcaus.data.y_test, clf.predict(classifcaus.data.x_test[:, :-1])))

proba= clf.predict_proba(classifcaus.data.x_test[:, :-1])[:,1]
y = (proba > 0.5) * 1.0
print(y == clf.predict(classifcaus.data.x_test[:, :-1]))"""
"""
Bench = BenchmarkClassif(["ClassCaus","lgbm", "xgb", "rf", "knn", "mlp", "dt", "lgr"])
Bench.evall_all_bench(params_classifcaus)
"""
